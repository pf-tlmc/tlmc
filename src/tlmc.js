...
const TLMC_SERVE_URL = process.argv[3] || process.env.TLMC_SERVE_URL || 'http://serve.tlmc.pf-n.co'
...
function createServer () {
  ...
  function parsePath (songPath) {
    const split = songPath.split('/')
  ...
  app.get('/tlmc/id/:id', (req, res) => {
    ...
    request.get(`${TLMC_SERVE_URL}/tlmc/${parsePath(song.path)}`).pipe(res)
  })
